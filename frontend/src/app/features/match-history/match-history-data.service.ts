...
export class MatchHistoryDataService {
  ...
  private load(): Observable<MatchRecord[]> {
    const sortDesc = (m: MatchRecord[]) => [...m].sort((a, b) => b.date.localeCompare(a.date));

    // Backend mode: gated MatchRecord[] (footages included only when the caller
    // has ftp — enforced server-side).
    ...
    return this.loadRows().pipe(
      map((rows) =>
        sortDesc(rows.map(rowToMatchRecord).filter((r): r is MatchRecord => r !== null))
      ),
    );
  }
